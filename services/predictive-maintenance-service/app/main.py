...
from app.api.v1 import maintenance_router
from app.core.config import PredictiveMaintenanceServiceSettings

settings = PredictiveMaintenanceServiceSettings()
configure_logging(
...
async def lifespan(app: FastAPI):
    ...
    logger.info("predictive-maintenance-service starting up")
    yield

    await app.state.http_client.aclose()
    await app.state.db.dispose()
    logger.info("predictive-maintenance-service shut down cleanly")


def create_app() -> FastAPI:
    app = FastAPI(
        title="Predictive Maintenance Service",
        description="XGBoost-based failure-risk prediction with root-cause attribution.",
